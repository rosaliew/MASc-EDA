...
- ``podp_bgc_ms2_links.csv``           one row per BGC-MS2 link, BGC <-> spectrum.
  ...
  not a curated set, despite its former name "ground_truth.csv".
- ``podp_genome_metabolome_pairs.csv`` one row per genome-metabolome link.
  All 4,966 declared pairs (was "strain_pairings.csv"). Declared co-origin
  only; carries no validation claim.
...
from scripts.podp.podp_online_file_locator import (  # noqa: E402
...
def build_funnel(
    ...
    stages = [
        ...
        ("BGC-MS2 links total", len(link_rows), "podp_bgc_ms2_links.csv rows"),
        ...
        ("strain pairings (untiered)", len(pairing_rows),
         "podp_genome_metabolome_pairs.csv rows -- declared co-origin only"),
    ]
...
def main() -> int:
    ...
    write_csv(args.out_dir / "podp_bgc_ms2_links.csv", link_rows)
    write_csv(args.out_dir / "podp_genome_metabolome_pairs.csv", pairing_rows)
    if report:
    ...
    print(f"\nwrote podp_bgc_ms2_links.csv           ({len(link_rows)} rows)")
    print(f"wrote podp_genome_metabolome_pairs.csv ({len(pairing_rows)} rows)")
    return 0
